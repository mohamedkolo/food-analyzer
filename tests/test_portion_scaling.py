...
def test_each_single_day_lands_on_its_own_target_with_cycling_on():
    ...
    ‏الخطة بتتولد بعشوائية مربوطة بالوقت (plan_engine بيعمل seed من
    datetime.now)، فـrandom.seed هنا مابيثبّتهاش -- الاختبار ده عيّنة
    مختلفة كل مرة، مش حالة واحدة ثابتة. فالحدود اللي تحت مقيسة على
    ٣٣٦٠ يوم، مش مفترضة:

        قبل رد الباقي على الوجبات اللي بتتحرك:  أسوأ ١٣.٨٪، و٣ أيام فوق ١٠٪
        بعده:                                  أسوأ  ٩.٢٪، وصفر يوم فوق ١٠٪

    ‏فبنسمح بيوم واحد من ١٦٨ يعدّي ١٠٪ (عشان العيّنة عشوائية)، ومابنسمحش
    بأي يوم يعدّي ١٥٪ خالص. اليوم اللي بيوصل لحدوده دايماً نفسه: أقل يوم
    في التدوير (١٢٠٠ كالوري) في نظام وجباته قليلة، لأن وجبة مش بتتحرك
    ("٢ بيض" مش بتبقى ١.٨) بتبقى نص اليوم.
    """
    random.seed(3)
    worst = (0.0, None)
    over = 0
    ...
    assert over <= 1, (
        "‏%d يوم من %d فرقهم أكبر من ١٠٪ -- المقيس صفر. أسوأ: %.0f%% %s"
        % (over, days, worst[0], worst[1]))
    assert worst[0] <= 15, (
        "‏يوم بعيد ١٥٪ عن هدفه: %.0f%% %s" % (worst[0], worst[1]))
